...
export default function BannerAgriStyle() {
  ...
  const goToBanner = (index: number) => {
    ...
    if (sliderRef.current) {
      sliderRef.current.style.transition = "transform 800ms ease-in-out"
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[400px] md:h-[500px] lg:h-[600px] w-full overflow-hidden">
        ...
        <div
          ref={sliderRef}
          className="absolute flex h-full w-full transition-transform duration-800 ease-in-out"
          style={{
          ...
          {bannerItems.map((item, index) => (
            <div key={item.id} className="relative h-full" style={{ width: `${100 / bannerItems.length}%` }}>
              ...
              <div className="absolute inset-0 overflow-hidden">
                <div
                  className={cn(
                    "absolute inset-0 w-[110%] h-[110%] transition-transform duration-[8000ms] ease-out",
              ...
              <div className="absolute inset-0 flex flex-col justify-center px-8 md:px-16 lg:px-24 max-w-3xl">
                <div className="overflow-hidden">
                  <h2
                    className={cn(
                      "text-3xl md:text-4xl lg:text-5xl font-bold text-white transition-all duration-1000 transform",
                ...
                <div
                  className={cn(
                    "w-20 h-1 bg-[#D40F14] my-6 transition-all duration-1000 delay-200 transform origin-left",
                ...
                <div className="overflow-hidden mb-8">
                  <p
                    className={cn(
                      "text-lg md:text-xl text-white/90 transition-all duration-1000 delay-400 transform",
                      index === activeIndex ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0",
                    )}
                  >
                    {item.description}
                ...
                <div>
                  <a
                    href={item.ctaLink}
                    className={cn(
                      "inline-block bg-[#D40F14] hover:bg-[#B00D11] text-white font-medium py-2 px-6 rounded-full transition-all duration-1000 delay-600 transform",
                      index === activeIndex ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0",
                    )}
                  >
                    {item.ctaText}
