...
export default function BannerRTLContinuous() {
  ...
  useEffect(() => {
    ...
    // Continuous right-to-left scrolling effect
    scrollIntervalRef.current = setInterval(() => {
      setScrollPosition((prev) => {
        // Move from 0 to 100 over the duration of the slide
        const newPosition = prev + (updateInterval / slideIntervalMs) * 100
  ...
  return (
    ...
    >
      ...
      <div className="relative h-[400px] md:h-[500px] lg:h-[600px] w-full">
        {/* Current slide with continuous movement */}
        <div
          className={cn(
          ...
          style={{
            transform: isAnimating ? "translateX(-100%)" : `translateX(-${scrollPosition * 0.05}%)`, // Subtle continuous movement
          }}
        >
          <div className="absolute inset-0 w-[105%]">
            {" "}
          ...
          {/* Text content with continuous movement */}
          <div
            className="absolute inset-0 flex flex-col justify-center px-8 md:px-16 lg:px-24 max-w-3xl"
            style={{
              transform: isAnimating ? "translateX(0)" : `translateX(-${scrollPosition * 0.2}%)`, // Text moves faster than background
            }}
          >
            <div className="overflow-hidden mb-4">
              <h2
                className={cn(
                  ...
                  !isAnimating ? "opacity-100" : "opacity-0 -translate-x-8",
                )}
              >
        ...
        <div
          className={cn(
            "absolute inset-0 transition-transform duration-800 ease-in-out",
            isAnimating ? "opacity-100" : "translate-x-full opacity-0",
          )}
          style={{
